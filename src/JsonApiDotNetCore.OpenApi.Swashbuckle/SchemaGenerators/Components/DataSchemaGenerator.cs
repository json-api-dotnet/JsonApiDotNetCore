        ...
        ResourceFieldSchemaBuilder fieldSchemaBuilder, SchemaRepository schemaRepository)
    {
        ...
        else
        {
            ...
            if (commonFieldsSchemaType == null && resourceSchemaTypeForFields.ResourceType.IsPartOfTypeHierarchy())
            {
                if (resourceSchemaTypeForFields.ResourceType.BaseType == null)
                {
                    OpenApiSchema referenceSchema = _resourceTypeSchemaGenerator.GenerateSchema(resourceSchemaTypeForFields.ResourceType, schemaRepository);

                    fullSchemaForFields.Properties.Add(OpenApiMediaTypeExtension.FullyQualifiedOpenApiDiscriminatorPropertyName,
                        referenceSchema.WrapInExtendedSchema());

                    fullSchemaForFields.Required.Add(OpenApiMediaTypeExtension.FullyQualifiedOpenApiDiscriminatorPropertyName);
                }
