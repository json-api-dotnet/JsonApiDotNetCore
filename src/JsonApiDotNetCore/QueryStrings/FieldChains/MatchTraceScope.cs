...
internal sealed partial class MatchTraceScope : IDisposable
{
    ...
    [LoggerMessage(Level = LogLevel.Trace, Message = "Start matching pattern '{Pattern}' against the complete chain '{Chain}'.")]
    private partial void LogMatchFirst(FieldChainPattern? pattern, string chain);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Start matching pattern '{Pattern}' against the remaining chain '{Chain}'.")]
    private partial void LogMatchNext(string indent, FieldChainPattern? pattern, string chain);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Match pattern '{Pattern}' against the chain '{Chain}': Success.")]
    private partial void LogMatchSuccess(string indent, FieldChainPattern? pattern, string chain);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Match pattern '{Pattern}' against the chain '{Chain}': Failed.")]
    private partial void LogMatchFailed(string indent, FieldChainPattern? pattern, string chain);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Backtracking to successful match against '{Chain}'.")]
    private partial void LogBacktrack(string indent, string chain);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Matching completed with success.")]
    private partial void LogCompletionSuccess(string indent);

    [LoggerMessage(Level = LogLevel.Trace, Message = "{Indent}Matching completed with failure.")]
