...
namespace JsonApiDotNetCore.Hooks.Internal
{
    ...
    internal sealed class ResourceHookExecutor : IResourceHookExecutor
    {
            ...
            where TResource : class, IIdentifiable
        {
            ...
            {
                IEnumerable filteredUniqueSet = CallHook(nextContainer, ResourceHook.OnReturn, new object[]
                {
                    nextNode.UniqueResources,
                    pipeline
                });
            ...
            where TResource : class, IIdentifiable
        {
            ...
            {
                CallHook(nextContainer, ResourceHook.AfterRead, new object[]
                {
                    nextNode.UniqueResources,
                    pipeline,
                    true
        ...
        private void Traverse(NodeLayer currentLayer, ResourceHook target, Action<IResourceHookContainer, IResourceNode> action)
        {
            while (true)
            {
                ...
                {
                    Type resourceType = node.ResourceType;
                    IResourceHookContainer hookContainer = _executorHelper.GetResourceHookContainer(resourceType, target);
            ...
            foreach (IResourceNode node in layer)
            {
                ...
                if (nestedHookContainer != null)
                {
                    if (uniqueResources.Cast<IIdentifiable>().Any())
                    {
                        ...
                        IEnumerable<string> allowedIds = CallHook(nestedHookContainer, ResourceHook.BeforeUpdateRelationship, new object[]
                        {
                            GetIds(uniqueResources),
            ...
            ResourcePipeline pipeline, IEnumerable existingImplicitResources = null)
        {
            ...
            CallHook(container, ResourceHook.BeforeImplicitUpdateRelationship, new object[]
            {
                resourcesByRelationship,
                pipeline
            });
        ...
        private void FireAfterUpdateRelationship(IResourceHookContainer container, IResourceNode node, ResourcePipeline pipeline)
        {
            ...
            CallHook(container, ResourceHook.AfterUpdateRelationship, new object[]
            {
                resourcesByRelationship,
                pipeline
            });
