...
internal sealed class DapperFacade
{
    ...
    public CommandDefinition? BuildSqlCommandForUpdate<TId>(ResourceChangeDetector changeDetector, [DisallowNull] TId leftId, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(changeDetector);
        ArgumentNullException.ThrowIfNull(leftId);
