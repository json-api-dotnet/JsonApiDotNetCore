...
public sealed class AllowSetTests : IClassFixture<OpenApiTestContext<OpenApiStartup<CapabilitiesDbContext>, CapabilitiesDbContext>>
{
    ...
    public async Task Hides_readonly_attribute_property_in_response()
    {
        ...
        const string fieldReadWriteName = "attributeReadWrite";

        document.Should().ContainPath("components.schemas.attributesInAllowSetCapabilityResponse.allOf[1].properties").With(propertiesElement =>
        {
            propertiesElement.Should().ContainPath(fieldReadOnlyName);
            propertiesElement.Should().ContainPath(fieldReadWriteName);
        });

        document.Should().ContainPath("components.schemas.attributesInCreateAllowSetCapabilityRequest.allOf[1].properties").With(propertiesElement =>
        {
            propertiesElement.Should().NotContainPath(fieldReadOnlyName);
            propertiesElement.Should().ContainPath(fieldReadWriteName);
        });

        document.Should().ContainPath("components.schemas.attributesInUpdateAllowSetCapabilityRequest.allOf[1].properties").With(propertiesElement =>
        {
            propertiesElement.Should().NotContainPath(fieldReadOnlyName);
            propertiesElement.Should().ContainPath(fieldReadWriteName);
        });
