...
using OpenApiNSwagEndToEndTests.CustomRoutes.GeneratedCode;
using OpenApiTests;
...
namespace OpenApiNSwagEndToEndTests.CustomRoutes;
...
public sealed class CustomRouteTests : IClassFixture<IntegrationTestContext<OpenApiStartup<CustomRouteDbContext>, CustomRouteDbContext>>, IDisposable
{
    ...
    public async Task Can_get_primary_resources_at_custom_route()
    {
        ...
        var apiClient = new CustomRoutesClient(httpClient);

        // Act
        ElectionCollectionResponseDocument response = await apiClient.GetElectionCollectionAsync();
    ...
    public async Task Can_get_secondary_resources_at_custom_route()
    {
        ...
        // Act
        CandidateCollectionResponseDocument response = await apiClient.GetElectionContendersAsync(election.Id);
    ...
    public async Task Can_get_election_winner()
    {
        ...
        // Act
        PrimaryCandidateResponseDocument response = await apiClient.GetWinnerAsync(election.Id);
    ...
    public async Task Cannot_get_election_winner_for_unknown_election()
    {
        ...
        // Act
        Func<Task> action = async () => _ = await apiClient.GetWinnerAsync(Unknown.TypedId.Guid);

        // Assert
        ApiException<ErrorResponseDocument> exception = (await action.Should().ThrowExactlyAsync<ApiException<ErrorResponseDocument>>()).Which;
        exception.StatusCode.Should().Be((int)HttpStatusCode.NotFound);
        exception.Message.Should().Be("HTTP 404: The election does not exist.");
    ...
    public async Task Cannot_get_election_winner_when_no_votes()
    {
        ...
        // Act
        Func<Task> action = async () => _ = await apiClient.GetWinnerAsync(election.Id);

        // Assert
        ApiException<ErrorResponseDocument> exception = (await action.Should().ThrowExactlyAsync<ApiException<ErrorResponseDocument>>()).Which;
        exception.StatusCode.Should().Be((int)HttpStatusCode.Conflict);
        exception.Message.Should().Be("HTTP 409: No single winner found.");
        exception.Result.Errors.Should().HaveCount(1);
