...
namespace JsonApiDotNetCoreExampleTests.IntegrationTests.ReadWrite.Updating.Relationships
{
    public sealed class AddToToManyRelationshipTests : IClassFixture<ExampleIntegrationTestContext<TestableStartup<ReadWriteDbContext>, ReadWriteDbContext>>
    {
        ...
        public async Task Can_add_to_HasManyThrough_relationship_with_already_assigned_resources()
        {
            ...
            existingWorkItems[0].WorkItemTags = new[] { new WorkItemTag { Tag = _fakers.WorkTag.Generate() } };
            existingWorkItems[1].WorkItemTags = new[] { new WorkItemTag { Tag = _fakers.WorkTag.Generate() } };
            ...
            var requestBody = new
            {
                data = new[]
                {
                    new { type = "workTags", id = existingWorkItems[0].WorkItemTags.ElementAt(0).Tag.StringId },
        ...
        public async Task Cannot_add_for_missing_type()
        {
            ...
            var requestBody = new { data = new[] { new { id = 99999999 } } };
        ...
        public async Task Can_add_with_duplicates()
        {
            ...
            var requestBody = new
            {
                data = new[]
                {
                    new { type = "userAccounts", id = existingSubscriber.StringId }, new { type = "userAccounts", id = existingSubscriber.StringId }
                }
            };
