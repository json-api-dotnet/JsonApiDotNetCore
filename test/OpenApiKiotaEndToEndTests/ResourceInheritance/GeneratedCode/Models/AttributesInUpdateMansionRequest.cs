// <auto-generated/>
using Microsoft.Kiota.Abstractions.Serialization;
using Microsoft.Kiota.Abstractions.Store;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System;
namespace OpenApiKiotaEndToEndTests.ResourceInheritance.GeneratedCode.Models
{
    #pragma warning disable CS1591
    public class AttributesInUpdateMansionRequest : IBackedModel, IParsable
    #pragma warning restore CS1591
    {
        /// <summary>Stores model information.</summary>
        public IBackingStore BackingStore { get; private set; }
        /// <summary>The numberOfResidents property</summary>
        public int? NumberOfResidents
        {
            get { return BackingStore?.Get<int?>("numberOfResidents"); }
            set { BackingStore?.Set("numberOfResidents", value); }
        }
        /// <summary>The ownerName property</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? OwnerName
        {
            get { return BackingStore?.Get<string?>("ownerName"); }
            set { BackingStore?.Set("ownerName", value); }
        }
#nullable restore
#else
        public string OwnerName
        {
            get { return BackingStore?.Get<string>("ownerName"); }
            set { BackingStore?.Set("ownerName", value); }
        }
#endif
        /// <summary>The surfaceInSquareMeters property</summary>
        public int? SurfaceInSquareMeters
        {
            get { return BackingStore?.Get<int?>("surfaceInSquareMeters"); }
            set { BackingStore?.Set("surfaceInSquareMeters", value); }
        }
        /// <summary>
        /// Instantiates a new <see cref="OpenApiKiotaEndToEndTests.ResourceInheritance.GeneratedCode.Models.AttributesInUpdateMansionRequest"/> and sets the default values.
        /// </summary>
        public AttributesInUpdateMansionRequest()
        {
            BackingStore = BackingStoreFactorySingleton.Instance.CreateBackingStore();
        }
        /// <summary>
        /// Creates a new instance of the appropriate class based on discriminator value
        /// </summary>
        /// <returns>A <see cref="OpenApiKiotaEndToEndTests.ResourceInheritance.GeneratedCode.Models.AttributesInUpdateMansionRequest"/></returns>
        /// <param name="parseNode">The parse node to use to read the discriminator value and create the object</param>
        public static OpenApiKiotaEndToEndTests.ResourceInheritance.GeneratedCode.Models.AttributesInUpdateMansionRequest CreateFromDiscriminatorValue(IParseNode parseNode)
        {
            _ = parseNode ?? throw new ArgumentNullException(nameof(parseNode));
            return new OpenApiKiotaEndToEndTests.ResourceInheritance.GeneratedCode.Models.AttributesInUpdateMansionRequest();
        }
        /// <summary>
        /// The deserialization information for the current model
        /// </summary>
        /// <returns>A IDictionary&lt;string, Action&lt;IParseNode&gt;&gt;</returns>
        public virtual IDictionary<string, Action<IParseNode>> GetFieldDeserializers()
        {
            return new Dictionary<string, Action<IParseNode>>
            {
                { "numberOfResidents", n => { NumberOfResidents = n.GetIntValue(); } },
                { "ownerName", n => { OwnerName = n.GetStringValue(); } },
                { "surfaceInSquareMeters", n => { SurfaceInSquareMeters = n.GetIntValue(); } },
            };
        }
        /// <summary>
        /// Serializes information the current object
        /// </summary>
        /// <param name="writer">Serialization writer to use to serialize this model</param>
        public virtual void Serialize(ISerializationWriter writer)
        {
            _ = writer ?? throw new ArgumentNullException(nameof(writer));
            writer.WriteIntValue("numberOfResidents", NumberOfResidents);
            writer.WriteStringValue("ownerName", OwnerName);
            writer.WriteIntValue("surfaceInSquareMeters", SurfaceInSquareMeters);
        }
    }
}
